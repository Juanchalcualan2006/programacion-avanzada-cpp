...
class MonitorDeVuelo {
    ...
    :registro(std::move(unRegistro)), idMonitor(unId) {
    std::cout << "Monitor " << idMonitor << " conectado" << std::endl;
        }
...
auto compartido = std::make_shared<RegistroDeVuelo>(5);
compartido->guardarAltura(0, 350.0);
std::cout << "compartido.use_count() = " << compartido.use_count() << std::endl;

    {
    MonitorDeVuelo torre_quito(1, compartido);
    std::cout << "compartido.use_count() = " << compartido.use_count() << std::endl;

    MonitorDeVuelo torre_ambato(2, compartido);
    std::cout << "compartido.use_count() = " << compartido.use_count() << std::endl;

    MonitorDeVuelo torre_cuenca(3, compartido);
    std::cout << "compartido.use_count() = " << compartido.use_count() << std::endl;
    ...
    std::cout << "compartido.use_count() tras el bloque = "
